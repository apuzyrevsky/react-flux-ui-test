...
server.post("/volumes/", function (req, res, next) {
  const error = validateVolume(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.name); // Generate a slug for new volumes.
    next();
  }
});
...
function validateVolume(volume) {
  if (!volume.name) return "Name is required.";
  if (!volume.type) return "Type is required.";
  if (!volume.size) return "Size is required.";
  return "";
}
